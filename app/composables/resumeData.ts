...
export const useResumeData: () => { data: ResumeData, avatar: File | null } = () => {
	return {
		data: {
			...
			skillCategories: useState("skillCategories", (): SkillCategory[] => [ { name: "", skills: [ { name: "" } ] } ]).value,
			links: useState("links", (): Link[] => [ { name: "", url: "" } ]).value,
			institutions: useState("institutions", (): Institution[] => [ { uuid: v7(), name: "" } ]).value,
			education: useState("education", (): Education[] => [ { degree: "", text: "" } ]).value,
			experience: useState("experience", (): Experience[] => [ { position: "", text: "", technologies: [] } ]).value,
			projects: useState("projects", (): Project[] => [ { name: "", description: "", url: "", repoLink: { name: "", url: "" }, technologies: [] } ]).value,
		},
